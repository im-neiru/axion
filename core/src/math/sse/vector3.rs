...
#[test]
fn test_Vector3_sse3() {
    use std::time::SystemTime;

    let a = Vector3::new(3.0, 4.0, 2.0);
    let b = Vector3::new(4.0, 3.2, 5.0);
    ...
    let ref_time = SystemTime::now();
    let is_zero = Vector3::ZERO.is_zero();
    ...
    let ref_time = SystemTime::now();
    let is_finite = Vector3::INFINITY.is_finite();
    ...
    // assert!(dot == 34.8, "Vector3::dot | Wrong output");
    // assert!(length == 5.0, "Vector3::length | Wrong output");

    println!("Vector3::dot | {span_dot} ns | {a} ⋅ {b} = {dot}",);

    println!("Vector3::length | {span_length} ns | {a} = {length}",);

    println!("Vector3::normalize | {span_normalize} ns | {a} = {normalize}",);

    println!("Vector3::distance | {span_distance} ns | {a} {b} = {distance}",);

    println!(
        "Vector3::is_nan | {span_is_nan} ns | {}  = {is_nan}",
        Vector3::NAN,
    );

    println!(
        "Vector3::is_nan | {span_is_zero} ns | {}  = {is_zero}",
        Vector3::ZERO,
    );

    println!(
        "Vector3::is_finite | {span_is_finite} ns | {}  = {is_finite}",
        Vector3::INFINITY,
    );

    {
        let start = Vector3::new(1.0, 2.0, 1.0);
        let end = Vector3::new(3.0, 4.0, 1.0);
        ...
        println!("Vector3::lerp | {span_lerp} ns | {start} {end} = {lerp}");
    }

    {
        ...
        println!("Vector3::eq | {span} ns | {a} == {b} = {eq}");
    }

    {
        ...
        println!("Vector3::ne | {span} ns | {a} != {b} = {ne}");
    }

    {
        ...
        println!("Vector3::distance_sq | {span} ns | {a} {b} = {distance_sq}");
    }

    {
        ...
        println!("Vector3::cross | {span} ns | {a} × {b} = {cross}");
    }
}
